...
// TODO need to add a way to specify a dir for the index and output it in a channel
// TODO add logic to run only if the index does not exist but output the index in a channel even if it exists
// of course also test that
...
// TODO here as well add a directory for the specific index and output it in a channel
// TODO add logic to run only if the index does not exist but output the index in a channel even if it exists
process STAR_INDEX {
    cpus 5
    memory '40 GB'
...
// TODO combine single and paired end process, retrieve the index from the previous process
